...
# ICSI
# lhotse download icsi --mic sdm $DATA_DIR/icsi
# lhotse prepare icsi --transcripts-dir $DATA_DIR/icsi/transcripts --mic sdm $DATA_DIR/icsi $MANIFESTS_DIR
...
# Rename ICSI manifests to match the naming convention
# for sfx in "supervisions" "recordings"; do
#     for partition in "train" "dev" "test"; do
#         mv $MANIFESTS_DIR/icsi-sdm_${sfx}_${partition}.jsonl.gz $MANIFESTS_DIR/icsi-sdm_${partition}_sc_${sfx}.jsonl.gz
#     done
# done
...
# metadata_dir=$DATA_DIR/tmp/LibriMix/metadata/Libri3Mix
...
#     --librimix_outdir $DATA_DIR/libri3mix \
#     --n_src 3 \
#     --freqs 16k \
#     --modes max \
...
#             --mixture_wavs_dir $DATA_DIR/libri3mix/Libri3Mix/wav16k/max/train-$n_hours/mix_$type \
#             --output_manifest $MANIFESTS_DIR/libri3mix_${type}_${n_hours}_train_sc_cutset.jsonl.gz
#         python $DATA_SCRIPTS_PATH/filter_long_cuts.py --input $MANIFESTS_DIR/libri3mix_${type}_${n_hours}_train_sc_cutset.jsonl.gz --output $MANIFESTS_DIR/libri3mix_${type}_${n_hours}_train_sc_cutset_30s.jsonl.gz --max_len 30
#     done
# done
...
#             --mixture_wavs_dir $DATA_DIR/libri3mix/Libri3Mix/wav16k/max/$partition/mix_$type \
#             --output_manifest $MANIFESTS_DIR/libri3mix_mix_${type}_sc_${partition}_cutset.jsonl.gz
#     done
# done
...
# Prepare cutsets
for rec_manifest in $(ls $MANIFESTS_DIR/icsi*recordings* ); do
...
python $DATA_SCRIPTS_PATH/pre_segment_using_alignments.py --input $MANIFESTS_DIR/icsi-sdm_train_sc_cutset.jsonl.gz --output $MANIFESTS_DIR/icsi-sdm_train_sc_cutset_30s.jsonl.gz --max_len 30
